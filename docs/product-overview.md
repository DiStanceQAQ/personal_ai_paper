...
- 查看某篇论文的 cards。
- 删除 cards。
- 对已解析论文执行 Agent 深度分析，生成带来源证据的 AI cards。

AI 生成的 cards 会记录来源 passage、证据 quote、分析 run ID 和质量标记。用户可以继续编辑或删除这些 cards；被用户编辑过的 AI card 会转为用户拥有，后续重新分析不会覆盖。
...
需要注意的是，AI card 生成依赖已配置的 LLM 和可追溯来源证据；PDF 解析对复杂双栏、表格、公式和扫描件的支持有限；agent 接入应严格依赖 active space 和来源可追溯结果，避免跨项目上下文污染。
